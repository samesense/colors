...
```{r, fig.height=30}
af = '../data/end/dist.tsv'
mat <- read_tsv(af, show_col_types = FALSE) |>
  select(iterm_name, nvimTheme, similarity_score) |>
  pivot_wider(names_from = nvimTheme, 
              values_from = similarity_score) %>%
  as.data.frame()
...
pheatmap(
  mat,
  cluster_rows = TRUE,
  cluster_cols = FALSE,   # keep your nvimTheme order
  scale = "none",
  main = "Similarity Scores (lower = better)",
  color = colorRampPalette(c("darkgreen", "yellow", "red"))(100),
  fontsize_row = 10,
  fontsize_col = 12
)
```
...
mat <- read_tsv(af, show_col_types = FALSE) |>
  ...
  pivot_wider(names_from = nvim_name, 
              values_from = similarity_index_lab) %>%
  as.data.frame()
...
pheatmap(
  ...
  cluster_cols = TRUE,
  scale = "none",
  main = "Similarity Scores (higher = better)",
...
best_pairs  <- pair_dat %>% arrange(desc(sim)) %>% head(10)
...
best_pairs_tbl <- best_pairs %>%
  gt() %>%
  tab_header(title = md("**Top 10 Best Matches (Neovim ↔ iTerm)**")) %>%
